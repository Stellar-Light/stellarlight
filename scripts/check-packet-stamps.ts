...
/**
 * Framework defaults. A page that still ships its scaffold's title has not been
 * deployed as a product — sorosan's dapp answers "Create Next App" with the
 * description "Generated by create next app", and its org has been silent since
 * February 2024. These must never count as the page naming itself.
 */
...
/** The head a client-rendered page serves BEFORE its bundle runs. */
export function headMeta(html: string): { title: string; description: string } {
	const t = /<title[^>]*>([\s\S]*?)<\/title>/i.exec(html)?.[1] ?? "";
	const d =
		/<meta[^>]+name=["']description["'][^>]+content=["']([^"']{2,300})/i.exec(
			html,
		)?.[1] ??
		/<meta[^>]+property=["']og:description["'][^>]+content=["']([^"']{2,300})/i.exec(
			html,
		)?.[1] ??
		"";
	return { title: readableText(t), description: readableText(d) };
}

/** Does the head NAME something, rather than being empty or a scaffold? */
export function headNamesAProduct(meta: {
	title: string;
	description: string;
}): boolean {
	const t = meta.title.trim().toLowerCase();
	if (!t) return false;
	if (SCAFFOLD_TITLES.includes(t)) return false;
	// A bare hostname is what a parked page serves; it names a domain, not a
	// product. "sorosplits.xyz" or "jetprotocol.io/lander" must not pass.
...
export function judgeStamp(p: {
	...
	if (text.length <= 300 && /<script\b/i.test(p.html)) {
		...
		if (isAppStore(p.sourceUrl))
			return {
				verdict: "COULD-NOT-CHECK",
				reason: `store listing unreadable and the lookup API did not answer (${text.length} chars) — retry, not a verdict`,
			};
		// Before giving up: a client-rendered page still serves a HEAD, written
		// by the operator's own deployment. Measured across the 20 rows stuck in
		// this state on 2026-09-07, 15 carry a <title> naming the product —
		// albedo, obsrvr, StellarBroker, Borderless — so the guard was reporting
		// "could not look" at pages that say who they are.
		//
		// The dead markers run over the head too: once the body is a stub, a
		// parked page's notice is often only in its title.
		const meta = headMeta(p.html);
		const headMarker = findMarker(
			`${meta.title} ${meta.description}`.toLowerCase(),
		);
		if (headMarker)
			return p.to === "Inactive"
				? { verdict: "HOLDS", reason: `head still says "${headMarker}"` }
				: {
						verdict: "CONTRADICTED",
						reason: `the page's own head says "${headMarker}" — "${meta.title.slice(0, 60)}"`,
					};
		if (headNamesAProduct(meta))
			return {
				verdict: "HOLDS",
				reason: `body is client-rendered, but the served head names the product: "${meta.title.slice(0, 70)}"`,
			};
		return {
			verdict: "COULD-NOT-CHECK",
			reason: meta.title
				? `client-rendered shell whose head names nothing usable ("${meta.title.slice(0, 40)}") — needs a browser`
				: `client-rendered shell (${text.length} chars readable, no head to read) — needs a browser`,
		};
	}
