...
export function AskSearch() {
	...
	const run = useCallback(async (query: string) => {
		...
		try {
			const [rRes, pRes, partRes] = await Promise.all([
				fetch(`/api/research?q=${encodeURIComponent(query)}&limit=6`).then(
					(r) => (r.ok ? r.json() : { results: [] }),
				),
				fetch(
					`/api/projects/search?q=${encodeURIComponent(query)}&limit=6`,
	...
	return (
		<div className="w-full">
			...
			{answered &&
				!loading &&
				(partners.length > 0 || projects.length > 0 || research.length > 0) && (
					<div className="mt-10 space-y-10">
									...
									<div className="rounded-xl bg-card border border-border p-4">
										<p className="text-sm text-foreground/90 leading-relaxed">
											{answer.text.split(/(\[\d+\])/).map((part, i) => {
												...
												if (!m)
													return <span key={`t-${i.toString()}`}>{part}</span>;
						...
						<p className="text-[11px] text-muted-foreground/60 pt-2">
							grounded results from the live stellarlight index — semantic
							search over the knowledge corpus + project directory. every card
