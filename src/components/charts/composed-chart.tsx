...
function ChartInner({
	...
	return (
		<TimeSeriesChartInner
			...
			// House patch (stellarlight): when bars are present, inset the x range
			// by half the bar width (+2px breathing) so the first/last columns sit
			// fully inside the plot instead of overhanging the axis labels.
			xRangeInset={
				barDataKeys.length > 0
					? Math.ceil((barSize ?? maxBarSize ?? 24) / 2) + 2
					: 0
			}
