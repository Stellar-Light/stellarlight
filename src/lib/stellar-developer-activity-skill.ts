/**
 * Inlined Stellar Developer Activity Skill manifest.
 *
 * Kept in sync with public/skills/stellar-developer-activity.md (the
 * publicly-hosted copy). Inlined here so the "Skill" copy button on
 * /leaderboard can write to the clipboard synchronously without
 * fetching, which avoids Safari's user-activation race that made
 * fetch-then-writeText fall through to the new-tab fallback.
 */
export const STELLAR_DEVELOPER_ACTIVITY_SKILL = `---
...
|---|---|---|---|
| \`sort\` | string | \`activity\` | \`activity\`, \`stars\`, \`issues\` |
...
- Headline stats: \`GET /api/leaderboard\` → read \`.ecosystem.*\`
- Active DeFi this month: \`GET /api/leaderboard?category=Protocol/Contract&range=30d&sort=activity&limit=10\`
- SCF-funded only: \`GET /api/leaderboard?limit=300\` → filter \`.projects[*]\` where \`scfAwarded === true\`

Each project has a public profile at \`https://stellarlight.xyz/project/{slug}\`.

## Data freshness

- Project metadata: curator-edited, refreshed continuously
- GitHub signals: refreshed by a daily cron job (~24h)
- Ecosystem dev stats (Electric Capital snapshot): refreshed daily at 06:00 UTC

## Limitations

- Only includes projects curated into stellarlight (~300); not every Stellar repo on GitHub.
- "Active dev" = at least one commit to a Stellar ecosystem repo in the trailing 28 days.
- Country breakdown only covers the ~23% of active devs who publish a public location on GitHub.

## Attribution

Attribute to **Electric Capital — Open Dev Data** (ecosystem stats) and **stellarlight.xyz** (project leaderboard).
`;
