...
function getAvatarUrls(project: any): { source: string; url: string }[] {
  ...
  if (twitter) {
    urls.push({ source: `x/${twitter}`, url: `https://unavatar.io/x/${twitter}` });
  }

  // GitHub avatars — sometimes generic user icons, so lower priority
  const github = extractGithubUsername(project);
  if (github) {
    urls.push({ source: `github/${github}`, url: `https://github.com/${github}.png?size=200` });
  }
...
async function main() {
  ...
  console.log(`Force: ${force ? "YES (re-fetch all)" : "NO (skip projects with logos)"}`);
  console.log("");
  ...
  for (const project of projects) {
    ...
    for (const { source, url } of sources) {
      ...
      try {
        ...
        console.log(`  OK: ${name} ← ${source} (${(imageData.buffer.length / 1024).toFixed(1)}KB)`);
        stats.fetched++;
        fetched = true;
        break;
      } catch (err) {
        console.error(`  UPLOAD ERROR: ${name} ← ${source}: ${err instanceof Error ? err.message : String(err)}`);
      }
    }

    if (!fetched) {
      stats.failed++;
      failures.push(`${name}: all sources failed (tried: ${sources.map((s) => s.source).join(", ")})`);
