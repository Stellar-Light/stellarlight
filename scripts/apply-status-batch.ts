...
const OPS: [string, string, string][] = [
	[
		"zenex",
		"Pre-Release",
		"testnet, not mainnet — was overclaiming Live (user-confirmed)",
	],
	[
		"vibrant",
		"Draft",
		"rebranded to Vesseo; retire stale-brand record (canonical = vesseo)",
	],
...
async function main() {
	...
	for (const [slug, status, reason] of OPS) {
		...
		if (!doc) {
			console.log(`  ⚠ NOT FOUND: ${slug} — skip (${reason})`);
			skipped++;
			continue;
		}
		if (doc.status === status) {
			console.log(`  • ${doc.name} (${slug}) already ${status} — skip`);
			skipped++;
			continue;
		}
		console.log(
			`  ${doc.name} (${slug}): ${doc.status} → ${status}   [${reason}]`,
		);
		if (EXECUTE) {
