...
async function main() {
	...
	for (const [band, list] of [["DEEP", deep], ["SHALLOW", shallow]] as const) {
		for (const full of list) {
			try {
				...
				rows.push({ full, band, depth: d.codeDepth, nt: d.nonTrivialFns, crates: d.contractCrates, sloc: d.rustSloc, m: d.cloneMultiplier, proof: r.proof });
			} catch (e) {
	...
	if (deepScores.length && shallowScores.length) {
		...
		console.log(`deep clears 0.6 gate: ${deepScores.filter((s) => s >= 0.6).length}/${deepScores.length}  shallow below: ${shallowScores.filter((s) => s < 0.6).length}/${shallowScores.length}`);
	}
}
