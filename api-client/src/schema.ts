...
export interface components {
    schemas: {
        ...
        Repo: {
            ...
            /**
             * @description What KIND of repo this is, DERIVED at read time from the row's own stored signals (nothing new is stored or researched), first match wins: isArchived → archived; isFork → fork; judgedHackathon → hackathon (a judged hackathon submission, a demo until other evidence says otherwise); a template/example/tutorial-looking name → template-or-tutorial; codeVerified.isDeployableContract → contract; linked to a directory product (project) → application; else code — a hackathon demo, a fork and a shipped product are not equal references, so weigh it by kindBasis.
             * @enum {string}
             */
            kind?: "archived" | "fork" | "hackathon" | "template-or-tutorial" | "contract" | "application" | "code";
