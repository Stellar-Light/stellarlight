...
async function testStellarPassportAPI() {
	...
	if (!apiKey) {
		console.error(
			"STELLAR_PASSPORT API key not found in environment variables",
		);
		process.exit(1);
	}
	...
	try {
		...
		const response = await fetch(
			"https://demo.stellarpassport.xyz/api/v1/profiles",
			{
				headers: {
		...
		const builderResponse = await fetch(
			`https://demo.stellarpassport.xyz/api/v1/profiles/alexanderkoh`,
			{
				headers: {
					Authorization: `Bearer ${apiKey}`,
					"Content-Type": "application/json",
		...
		if (builderResponse.ok) {
			...
			console.log(
				"Single Builder Response:",
				JSON.stringify(builderData, null, 2),
			);
		} else {
			console.error(
				`Failed to fetch specific builder: ${builderResponse.status}`,
			);
		}
	} catch (error) {
