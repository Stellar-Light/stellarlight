...
async function main() {
	...
	for (const u of data.descUpdates) {
		...
		console.log(
			`  ${doc.name} (${u.slug}): "${u.shortDescription.slice(0, 64)}…"`,
		);
		if (EXECUTE)
			await payload.update({
				collection: "projects",
				id: doc.id,
				data: { shortDescription: u.shortDescription, ...protect(doc) },
			});
	}
	...
	for (const t of data.retags) {
		...
		if (t.scfAwarded !== undefined)
			patch.scf = {
				...(doc.scf || {}),
				awarded: t.scfAwarded,
				...(t.scfTotalAwardedUSD !== undefined
					? { totalAwarded: t.scfTotalAwardedUSD }
					: {}),
			};
		console.log(
			`  ${doc.name} (${t.slug}): ${JSON.stringify({ category: t.category, scf: t.scfAwarded, usd: t.scfTotalAwardedUSD })}`,
		);
		if (EXECUTE)
			await payload.update({ collection: "projects", id: doc.id, data: patch });
	}
	...
	for (const n of data.newRecords) {
		...
		if (await find(slug)) {
			console.log(`  • ${n.name} (${slug}) already exists — skip`);
			continue;
		}
