					...
					productsCoverage: {
						...
						properties: {
							basis: {
								...
								description:
									"issuer-stellar-toml = classic assets, compared against what the issuer accounts' own stellar.toml declares. deployer-contracts = Soroban tokens (no toml), compared against every contract the entity's deployer account created with its token wasm, read from Horizon's create-contract history — the issuer's own act, and wider than rwa.xyz's listing (Spiko: 17 token contracts deployed, 9 listed, 8 with zero supply tracked as deployed-no-supply).",
							},
							asOf: { type: "string" },
							issuers: { type: "integer" },
