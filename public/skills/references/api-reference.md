...
## `GET /api/verify`
**Claim verification** — a claim in, a verdict out, with dated evidence attached. `?claim=is blend audited` / `is laina live` / `is kulipa maintained` (closed grammar) or `?type=audited|live|maintained&subject=<name>[&auditor=][&since=]`. Four verdicts asserting over **our corpus, never the world**: `supported`, `contradicted` (we hold a dated record saying otherwise — e.g. a Pre-Release status record vs a live claim, with source), `unsupported` (no evidence either way — the statement carries the denominator and does NOT mean false), `unresolved`. Evidence rows are typed (`audit-report` | `status-record` | `code-activity` | `curated-note`) — discriminate on `kind`. Every answer carries the full **subject card** (links, types, status+provenance, prominence), so relay it rather than re-fetching. Audit answers may carry `currencyNote` (newest report predates latest code activity); auditor-filtered misses list `auditorsOnRecord`. Prefer this over interpreting search results for is-it-audited / is-it-live / is-it-maintained questions. **Params:** `claim` OR `type`+`subject`, optional `auditor`, `since`.
