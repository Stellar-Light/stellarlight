...
async function main() {
	...
	if (!doc) { console.error(`No project with slug "${SLUG}"`); process.exit(1); }
	...
	console.log(`    verificationLevel: ${doc.verificationLevel} → ${newVerif}   [sync-protect]`);
	if (!EXECUTE) { console.log("\nDRY RUN — no changes."); process.exit(0); }

	await payload.update({
		collection: "projects",
		id: doc.id,
		data: { status: STATUS, verificationLevel: newVerif, provenance: { ...(doc.provenance || {}), source: "AdminEdit" } },
	});
