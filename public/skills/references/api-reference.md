...
## `GET /api/verify`
**Claim verification (v1: audit claims)** — a claim in, a verdict out, with dated evidence attached. `?claim=is blend audited` (closed grammar) or `?type=audited&subject=blend[&auditor=][&since=]`. Three verdicts, and they assert over **our corpus, never the world**: `supported` (reports on record — relay `statement` verbatim, it carries dates), `unsupported` (nothing on record — the statement carries the denominator and explicitly does NOT mean "unaudited"), `unresolved` (unknown subject). An auditor-filtered miss lists `auditorsOnRecord`; `currencyNote` appears when the newest report predates the project's latest code activity by >90 days — say so when recommending. Unknown claim types 400 with the supported forms. **Params:** `claim` OR `type`+`subject`, optional `auditor`, `since`.
