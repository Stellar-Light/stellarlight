...
const TYPES: Record<string, string[]> = {
	...
	getblock: ["RPC"],
	exaion: ["RPC"],
	"horizon-as-a-service": ["RPC"],
	obsrvr: ["RPC", "Indexer"],
	"lightsail-network-quasar": ["RPC", "Indexer"],
	friendbot: ["Faucet"],
	"stellar-cli": ["Faucet"],
	"stellar-quickstart": [],
	...
	bitgo: ["Security"],
	fordefi: ["Security"],
	"cactus-link": ["Security"],
	"ultra-stellar": ["Security"],
	// DEX / AMM
	soroswap: ["DEX"],
	aquarius: ["DEX"],
	phoenix: ["DEX"],
...
async function main() {
	...
	console.log(
		`Mode: ${EXECUTE ? "EXECUTE" : "DRY RUN"}  (${Object.keys(TYPES).length} records)\n`,
	);
	let done = 0,
		skipped = 0,
		missing = 0;
	for (const [slug, types] of Object.entries(TYPES)) {
		...
		if (cur === [...types].sort().join(",")) {
			console.log(`  • ${doc.name} (${slug}) already [${types}] — skip`);
			skipped++;
			continue;
		}
