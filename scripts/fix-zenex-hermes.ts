...
const protect = (d: any) => ({
	verificationLevel:
		d.verificationLevel === "Unverified"
			? "Verified (Community)"
			: d.verificationLevel,
...
async function main() {
	...
	const zenex: any = await payload.findByID({
		collection: "projects",
		id: ZENEX_ID,
		depth: 0,
	});
	const hermes: any = await payload
		.findByID({ collection: "projects", id: HERMES_ID, depth: 0 })
		.catch(() => null);
	console.log(`Mode: ${EXECUTE ? "EXECUTE" : "DRY RUN"}`);
	console.log(
		`  Zenex: name=${zenex?.name} scf.awarded=${zenex?.scf?.awarded} status=${zenex?.status}`,
	);
	console.log(
		`         desc="${String(zenex?.shortDescription).slice(0, 70)}…"`,
	);
	console.log(
		`  Hermes (dupe): ${hermes ? `name=${hermes.name} status=${hermes.status} scf=${hermes?.scf?.awarded}` : "NOT FOUND at that id"}`,
	);
	const newDesc = String(zenex.shortDescription || "").replace(
		/^\s*Hermes\b/,
		"Zenex (formerly Hermes)",
	);
	console.log(`  → Zenex desc becomes: "${newDesc.slice(0, 70)}…"`);
	console.log(
		`  → Zenex scf.awarded=true, totalAwarded=150000; Hermes → Draft`,
	);
	if (!EXECUTE) {
	...
	if (hermes)
		await payload.update({
			collection: "projects",
			id: HERMES_ID,
			data: { status: "Draft", ...protect(hermes) },
		});
	console.log(
		"DONE: Zenex tagged SCF + description cleaned; Hermes dupe retired.",
	);
	process.exit(0);
}
main().catch((e) => {
