...
export async function fetchAllBuilders(): Promise<PassportBuilder[]> {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch builders: ${response.status} ${response.statusText}`,
			);
		}
...
export async function fetchBuilder(
	...
	try {
		...
		if (!response.ok) {
			throw new Error(
				`Failed to fetch builder ${username}: ${response.status} ${response.statusText}`,
			);
		}
 ...
 */
export async function fetchActiveBuilders(
	limit: number = 10,
): Promise<PassportBuilder[]> {
	const allBuilders = await fetchAllBuilders();
	...
	return allBuilders
		...
		.sort((a, b) => {
			const aScore =
				(a.stats?.totalCommits30d || 0) + (a.stats?.activeDays30d || 0);
			const bScore =
				(b.stats?.totalCommits30d || 0) + (b.stats?.activeDays30d || 0);
