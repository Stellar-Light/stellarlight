...
async function verify(full: string, name: string): Promise<Found | null> {
	if (name.startsWith("@") && name.includes("/")) {
		...
		)) as { githubRepository?: { owner?: string; name?: string }; latestVersion?: string } | null;
		...
		for (const r of res.docs as unknown as Array<Record<string, unknown>>) {
			...
			if (found) {
				...
				if (samples.length < 20)
					samples.push(`  ${full} → ${found.name}@${found.version} (${found.registry})`);
			} else none++;
			...
			const same =
				...
				prev.every((p, i) => p?.name === next[i]?.name && p?.registry === next[i]?.registry);
			if (same) continue;
			if (EXECUTE) {
