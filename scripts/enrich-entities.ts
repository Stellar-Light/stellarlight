...
function getAvatarUrls(links: { twitter?: string; github?: string; website?: string }): { source: string; url: string }[] {
  const urls: { source: string; url: string }[] = [];

  if (links.twitter) {
    const user = extractUsername(links.twitter, /(?:twitter\.com|x\.com)\/([a-zA-Z0-9_]+)/i);
    if (user) urls.push({ source: `x/${user}`, url: `https://unavatar.io/x/${user}` });
  }

  if (links.github) {
    const user = extractUsername(links.github, /github\.com\/(?!orgs\/)([a-zA-Z0-9_-]+)/i);
    if (user) urls.push({ source: `github/${user}`, url: `https://github.com/${user}.png?size=200` });
  }

  if (links.website) {
    try {
      const parsed = new URL(links.website.startsWith("http") ? links.website : `https://${links.website}`);
...
async function main() {
  ...
  const entities: any[] = [];
  while (true) {
    const r = await payload.find({ collection: "entities", limit: 100, page, depth: 2 });
  ...
  for (const entity of entities) {
    ...
    const projects: any[] = Array.isArray(entity.projects) ? entity.projects : [];
    ...
    console.log(`  ${name} — best project: ${best.name} (score: ${scored[0].score})`);
    ...
    for (const { project: p } of scored) {
      if (!newLinks.website && p.links?.website) newLinks.website = p.links.website;
      if (!newLinks.github && p.links?.github) newLinks.github = p.links.github;
      if (!newLinks.twitter && p.links?.twitter) newLinks.twitter = p.links.twitter;
    }
    ...
    if (
      ...
      linksChanged = true;
      console.log(`    Links: website=${mergedLinks.website || "—"}, github=${mergedLinks.github || "—"}, twitter=${mergedLinks.twitter || "—"}`);
    }
    ...
    if (!entity.logo) {
        ...
        if (img) {
          try {
            ...
            const media = await payload.create({
              collection: "media",
              data: { alt: `${name} logo` },
              file: { data: img.buffer, name: filename, mimetype: img.contentType, size: img.buffer.length },
            });
            newLogoId = String(media.id);
            console.log(`    LOGO: ${source} (${(img.buffer.length / 1024).toFixed(1)}KB)`);
            stats.logoFromUnavatar++;
            gotAvatar = true;
            break;
          } catch (err) {
            console.log(`    LOGO UPLOAD ERROR: ${source}: ${err instanceof Error ? err.message : String(err)}`);
          }
        } else {
      ...
      if (!gotAvatar) {
        ...
        if (projectLogo) {
          const logoId = typeof projectLogo === "string" ? projectLogo : projectLogo.id;
          if (logoId) {
            newLogoId = logoId;
