/**
 * Inlined Stellar Hackathon Copilot SKILL.md.
 *
 * Mirror of public/skills/stellar-hackathon-copilot.md, kept in sync so
 * the copilot landing page's "Copy" button can write to the clipboard
 * synchronously (no fetch dependency, no Safari user-activation race).
 */
export const STELLAR_HACKATHON_COPILOT_SKILL = `
---
name: stellar-hackathon-copilot
description: A skill for deep Stellar ecosystem research. Helps hackers, founders, and grant applicants validate ideas before they build — surfaces prior art across Stellar hackathons, SCF rounds, and the broader project directory; recommends teammates from the Stellar Builders network; and points to the right SDK / skill track. Use when the user asks about who's building on Stellar, what's been tried, what won prizes, or whether they should build a specific idea.
---

# Stellar Hackathon Copilot

A research skill for **the strategic layer** of building on Stellar: what's been built, who's building it, what won prizes, and whether a new idea has a real gap to fill. For *how to build*, defer to the official Stellar Foundation skills at **https://skills.stellar.org/** (soroban, dapp, assets, data, agentic-payments, zk-proofs, standards).
